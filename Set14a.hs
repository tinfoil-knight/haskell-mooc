...
shout' []       = []
shout' [x]      = [T.toUpper x]
shout' (x:s:xs) = (T.toUpper x):s:(shout' xs)
...
countUtf8Chars b = case decodeUtf8' b of (Right t) -> Just (T.length t)
                                         (Left t)  -> Nothing
...
pingpong b = BL.cycle $ BL.append b' (BL.reverse b')
    where b' = BL.fromStrict b
