...
fmap2 fx v = fmap (\x -> fmap fx x) v

fmap3 :: (Functor f, Functor g, Functor h) => (a -> b) -> f (g (h a)) -> f (g (h b))
fmap3 fx v = fmap (\x -> fmap2 fx x) v
...
instance Functor TwoList where
  ...
  fmap f (TwoNode x y a) = TwoNode (f x) (f y) (fmap f a)
...
count c v = length $ filter (\x -> x == c) (toList v)
...
inBoth a b = filter (\x -> elem x $ (toList b)) (toList a)
...
instance Foldable List where
  ...
  foldr f init (LNode a x) = f a (foldr f init x)
