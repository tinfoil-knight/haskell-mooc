...
rangeOf (x:xs) = abs ((fst t) - (snd t)) where t = rangeOf' x x xs

rangeOf' :: (Ord a, Num a) => a -> a -> [a] -> (a,a)
rangeOf' biggest smallest []     = (biggest, smallest)
rangeOf' biggest smallest (x:xs) = rangeOf' maxA minA xs
                                    where maxA = max x biggest
                                          minA = min x smallest

-- Alt.
-- rangeOf xs = abs (head l - last l) where l = sort xs
...
-- Alt.
-- incrementKey d xs = map f xs
--                         where f t = if fst t == d
--                                       then (fst t, snd t + 1)
--                                       else t
...
winner scores player1 player2 =  if (f player2) > (f player1)
                                    then player2
                                    else player1
                                    where f k = Map.findWithDefault 0 k scores
...
freqs xs = Map.fromList [(i, count i xs) | i <- nub xs]
            where count n ns = (length . filter (==n)) ns
...
transfer from to amount bank
      ...
      | otherwise                                           = credit to (debit from bank)
                                                                  where credit r bk = Map.adjust (\x -> x + amount) r bk
                                                                        debit s bk = Map.adjust (\x -> x - amount) s bk
...
maxIndex arr = fst ((sortBy largest (assocs arr)) !! 0)
                  where largest x y = compare (snd y) (snd x)