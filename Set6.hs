...
instance Ord Country where
  compare a b = case a == b of
                True -> EQ
                False -> if a == Finland || b == Switzerland
                         then LT
                         else if a == Switzerland || b == Finland
                              then GT
                              else LT
...
instance Price Egg where
  price e = case e of ChickenEgg -> 20
                      ChocolateEgg -> 30
...
simplify (RationalNumber p q) = RationalNumber (p `div` g) (q `div` g)
                                  where g = hcf p q
